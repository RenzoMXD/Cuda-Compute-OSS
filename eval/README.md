...
```bash
# Reference regime: 1024, full-rank, 100 couples (all defaults), + scaling fit.
...
python -m eval --n 1024 --pairs 100 --rank-m 128 --sweep 512,1024,2048

# The strategy's happy path — compressible (low-rank) data, where it wins:
python -m eval --n 1024 --pairs 100 --fill lowrank --data-rank 16

# The accuracy floor defaults to 0.8; override it (or 0 to disable), emit JSON:
python -m eval --n 1024 --min-accuracy 0.9 --json
```
...
# reference regime: 1024, full-rank (fill defaults to "random")
out = evaluate(EvalConfig(n=1024, pairs=3, rank_m=128))
...
On the reference **full-rank** `1024` data *no* subspace of `M ≪ N` can
