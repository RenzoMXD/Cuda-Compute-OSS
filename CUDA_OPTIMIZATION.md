...
This document is **maintained by the optimization agent**. As kernels are optimized, the agent summarizes effective optimization strategies here, organized by kernel type. This serves as a growing knowledge base for future optimization runs.
...
1. **Maximize occupancy via grid sizing** (40% latency reduction):
...
2. **Inline helper functions to reduce register pressure** (registers: 96 → 39):
...
3. **Row-per-block launch instead of persistent threads** (~5% improvement):
...
4. **L2 eviction policy hints** (~1.5% improvement):
...
5. **Triton autotune for num_warps** (~1% improvement):
...
1. **Increase SeqTile from 2 to 4** (19% latency reduction):
...
2. **Float4 (16-byte) nope copy** (~1.3% additional improvement):
...
1. **Reduce tile size to cut register pressure** (27% total improvement):
...
2. **Non-persistent grid outperforms persistent for high tile count** (8% improvement):
...
3. **Increase grid to match SM block capacity** (17% improvement):
...
4. **L2 eviction hints** (~0.7% improvement):
...
1. **Remove .to(tl.float32) before tl.dot** (7.7x speedup):
...
2. **Expand autotune to proper GEMM tile sizes** (2x speedup on top of #1):
...
3. **Use tl.make_block_ptr for TMA loads** (~7% speedup):
...
4. **Non-persistent grid for moderate tile counts** (~3% speedup):
...
1. **FlashAttention-style Q-tiling** (8.4x speedup — most impactful):
...
2. **Remove redundant tl.where on attention weights** (12% speedup):
...
3. **Simplify causal mask** (3% speedup):
...
4. **Pre-computed loop bound** (7% speedup):
